...
string DTD::getRoot()
{
    ...
    while (elements.size()>1)
    {
        for(set<DeclarationElement *>::iterator itElt = elements.begin(); itElt != elements.end(); ++itElt)
        {
            ...
            ContenuChoix *contenuChoix = dynamic_cast<ContenuChoix *>((*itElt)->getContent());
            if (contenuChoix != NULL)
                nCorresponding += getNumCorrespondingChoice(contenuChoix, elements);

            ContenuSequence *contenuSequence = dynamic_cast<ContenuSequence *>((*itElt)->getContent());
            if (contenuSequence != NULL)
                nCorresponding += getNumCorrespondingSeq(contenuSequence, elements);

            ContenuSimple *contenuSimple = dynamic_cast<ContenuSimple *>((*itElt)->getContent());
...
{
    int nCorresponding = 0;
    list<Contenu *> *content = seq->getContent();
...
{
    int nCorresponding = 0;
    list<Contenu *> *content = choix->getContent();
